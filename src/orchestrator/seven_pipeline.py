...
class SevenPipelineResult:
    ...
    steel: Mapping[str, Any]
    civil: Mapping[str, Any]
    cross_discipline: Mapping[str, Any]
    blocking_flags: bool


class SevenDisciplinePipelineService:
    """Runs 7-discipline calculations and integrated cross-discipline checks."""
    ...
    def run(
        ...
        *,
        ...
        steel_input: Mapping[str, Any],
        civil_input: Mapping[str, Any],
    ) -> SevenPipelineResult:
        ...
        steel_result = self.orchestrator.handle_message(
            create_message(
                message_type="calculation_request",
                from_agent="orchestrator",
                to_agent="steel_specialist",
                payload={"calculation_type": "steel_integrity", "input_data": dict(steel_input)},
                correlation_id=run.trace.correlation_id,
                trace_id=run.trace.trace_id,
                meta={"workflow_id": "standard_calculation", "discipline": "steel"},
            )
        )
        civil_result = self.orchestrator.handle_message(
            create_message(
                message_type="calculation_request",
                from_agent="orchestrator",
                to_agent="civil_specialist",
                payload={"calculation_type": "civil_integrity", "input_data": dict(civil_input)},
                correlation_id=run.trace.correlation_id,
                trace_id=run.trace.trace_id,
    ...
    def _build_cross_payload(
        ...
        steel_result: Mapping[str, Any],
        civil_result: Mapping[str, Any],
    ) -> Dict[str, Any]:
        ...
        s_final = steel_result.get("results", {})
        c_final = civil_result.get("results", {})
        ...
        return {
            ...
            "steel": {
                "dc_ratio": s_final.get("dc_ratio"),
                "deflection_ratio": s_final.get("deflection_ratio"),
                "corrosion_loss_percent": s_final.get("corrosion_loss_percent"),
            },
            "civil": {
                "foundation_settlement_mm": c_final.get("foundation_settlement_mm"),
                "crack_width_mm": c_final.get("crack_width_mm"),
                "spalling_area_percent": c_final.get("spalling_area_percent"),
            },
        }
