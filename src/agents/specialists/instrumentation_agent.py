...
class InstrumentationSpecialistAgent:
    """Instrumentation specialist agent backed by instrumentation verification engine."""

    def __init__(self, service: InstrumentationVerificationService | None = None) -> None:
        self.service = service or InstrumentationVerificationService()

    def handle_calculation(self, payload: Dict[str, object], runtime: AgentRuntime) -> Dict[str, object]:
        _ = runtime
        calculation_type = str(payload.get("calculation_type", "instrumentation_integrity"))
