...
from src.api.server import dispatch_calculation, supported_disciplines
...
class ApiServerAdapterTests(unittest.TestCase):
    def test_supported_disciplines_are_seven(self) -> None:
        disciplines = supported_disciplines()
        self.assertEqual(len(disciplines), 7)
        self.assertIn("piping", disciplines)
        self.assertIn("civil", disciplines)

    def test_dispatch_returns_frontend_contract_for_each_discipline(self) -> None:
        for discipline, payload in sample_payloads().items():
            with self.subTest(discipline=discipline):
                result = dispatch_calculation(discipline, payload)
                ...
                self.assertIn("calculation_summary", result["details"])
                self.assertEqual(result["details"]["calculation_summary"]["discipline"], discipline)

    def test_invalid_discipline_raises_value_error(self) -> None:
        with self.assertRaises(ValueError):
            dispatch_calculation("invalid", {})
