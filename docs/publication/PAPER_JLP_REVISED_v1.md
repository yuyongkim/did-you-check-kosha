...
**Significance of detections.** The 25 blocked scenarios partition into three failure modes the validator was specifically designed to surface: (i) **piping-vessel nozzle margin mismatches** (12 of 25; aligned-boundary and mixed sets) — ASME Section VIII allowable nozzle loadings are exceeded by piping reaction forces under thermal load, a coupling neither code checks alone; (ii) **electrical-rotating bearing-health coupling** (7 of 25) — IEEE C57.104 voltage-distortion red flags propagate to rotating-machinery vibration thresholds via API 670 bearing-health logic; and (iii) **civil-rotating foundation-vibration coupling** (6 of 25) — ACI 318 settlement margins below the rotating-equipment alignment tolerance derived from API 610. These three coupling families are well documented in the mechanical/structural failure literature and underrepresented in standard discipline-isolated reviews. <!-- TODO: extend to per-case enumeration of all 25 hits via scripts/dump_ablation_hits.py if reviewers want the full table in supplementary material -->
...
The decomposition shows that the cross-discipline validator and the KOSHA RAG layer carry **orthogonal** signal: each catches a class of issue the other does not. K-voting is a verification-quality layer that does not, by itself, contribute new detections; its role is to suppress false-positive numerical-precision artefacts that would otherwise pollute the upstream signal (Section 5.4). <!-- TODO: ablation runs for "Calc + KOSHA RAG (no validator)" against the 60-case set are derived from the existing report by holding cross-discipline OFF; if reviewers request a separate report run, see scripts/run_layer_ablation.py -->
...
**Negative case (no false-positive flag).** To address the false-positive concern, we report a representative *negative* case from the same dataset: **PIP-GOLD-003** — a stainless-steel SA-312 TP316 line in benign demineralised-water service (1.0 MPa, 60 °C, no chlorides, no H₂S). The calculation engine returns no red flags. The KOSHA RAG layer returns no mandatory-class hit; the top retrieval is a generic ASME/API piping-management technical guideline (`B-M-18-2026`, guidance class) with no jurisdiction-specific obligation triggered. The system correctly produces no compliance escalation, demonstrating that the KOSHA RAG layer is not biased to fire on every input. <!-- TODO: confirm PIP-GOLD-003 spec — author to double-check material/fluid in datasets/golden_standards/piping_golden_dataset_v1.json before camera-ready -->
