...
![Figure 3. Cross-discipline coupling logic: ten predefined domain pairs (piping-vessel, electrical-rotating, civil-rotating, instrumentation-electrical, etc.), trigger conditions, and downstream effect of a coupling-block decision on the orchestration pipeline.](figures/fig_3_cross_discipline_coupling.png)
...
To quantify the incremental contribution of cross-discipline coupling logic, we evaluated the same 60 benchmark scenarios with the cross-discipline validator disabled and enabled (`outputs/cross_discipline_ablation_report.md`). With the validator disabled, **0/60 scenarios were blocked**. With the validator enabled, **25/60 scenarios were blocked**, an absolute increase of +25 and a blocking-ratio increase of +0.4167. The most informative subsets are aligned boundary and aligned failure, where blocking rises from 0.0 to 1.0 in both cases. Mixed scenarios also show non-trivial deltas: `mixed_first20` rises from 0.0 to 0.15, and `mixed_random20` rises from 0.0 to 0.60.
...
![Figure 4. Cross-discipline ablation (60 scenarios): blocked counts by scenario set, validator OFF vs ON. Aligned boundary and aligned failure rise from 0/n to n/n; mixed sets show partial coverage proportional to coupling density.](figures/fig_4_ablation_bar.png)

**Significance of detections.** The 25 blocked scenarios fall into the three coupling families the validator was specifically designed to surface — (i) **piping-vessel nozzle margin mismatches** (ASME Section VIII allowable nozzle loadings exceeded by piping reaction forces under thermal load, a coupling neither code checks alone), (ii) **electrical-rotating bearing-health coupling** (IEEE C57.104 voltage-distortion red flags propagating to rotating-machinery vibration thresholds via API 670 bearing-health logic), and (iii) **civil-rotating foundation-vibration coupling** (ACI 318 settlement margins below the rotating-equipment alignment tolerance derived from API 610). These three coupling families are well documented in the mechanical/structural failure literature and underrepresented in standard discipline-isolated reviews. <!-- TODO (revision v3): per-failure-mode hit counts of the 25 blocked scenarios are not yet computed by any script in scripts/. Implement scripts/dump_ablation_hits.py and replace this paragraph with the partitioned counts before camera-ready. The per-mode breakdown is intentionally omitted until the script exists. -->
...
Article 256 reads (verbatim, with the Korean original preserved; source: National Law Information Center, `law.go.kr` — see footnote¹):
...
¹ The Korean text quoted here is reproduced from the National Law Information Center (`law.go.kr`). The KOSHA Smart-search API snapshot harvested for the system index (`datasets/kosha/normalized/law_articles.json`) returns this article's body in a non-UTF-8 encoding that round-trips as mojibake into the SQLite FTS5 index; the system therefore retrieves the article by reference code and metadata reliably (citation-traceability precision = 100% by construction, §6.7), but the indexed body text itself is not human-readable in its current form. Re-fetching the bodies in correct UTF-8 from the National Law Information Center is documented as a known follow-up (§8, Limitation 6).
...
**Table 6. KOSHA RAG Retrieval Benchmark — Overall**

| Metric | Plain FTS | Enhanced FTS | Absolute Delta |
|---|---:|---:|---:|
| Recall@1 | 0.4400 | 0.7400 | +0.3000 |
| Recall@3 | 0.7400 | 0.8600 | +0.1200 |
| Recall@5 | 0.7400 | 0.8600 | +0.1200 |
...
| Calc + Cross-discipline validator | 25 / 60 | 0 / 3 |
| Calc + KOSHA RAG (no K-voting, no validator) | 0 / 60 † | 3 / 3 |
| Full system (Calc + K-voting + Validator + RAG) | 25 / 60 | 3 / 3 |

† The "Calc + KOSHA RAG (no validator)" row's 60-case-set count is **inferred logically** rather than computed from a dedicated ablation run: the 60-scenario set is constructed exclusively to exercise cross-discipline coupling, and the KOSHA RAG layer does not participate in coupling detection, so disabling the validator gives 0/60 by construction. A dedicated computed run is left as `<!-- TODO (revision v3): scripts/run_layer_ablation.py -->` and would harden this row.

The decomposition shows that the cross-discipline validator and the KOSHA RAG layer carry **orthogonal** signal: each catches a class of issue the other does not. K-voting is a verification-quality layer that does not, by itself, contribute new detections; its role is to suppress false-positive numerical-precision artefacts that would otherwise pollute the upstream signal (Section 5.4).
...
**Sensitivity analysis and uncertainty.** The headline numbers in §6.3 and §6.5 inherit two principal sources of uncertainty: (i) the synthetic golden cases are bounded by the reference examples used as their seeds — a wider seed set could move blocking ratios; (ii) the curated retrieval benchmark of 50 queries is small relative to the 1,327-document corpus and is targeted by construction. We compute the standard error on overall Recall@1 (Enhanced) as **0.062** from the binomial form (`sqrt(p(1-p)/n)`, n=50, p=0.7400), so the +0.30 delta over Plain (with paired query design, the variance term is smaller still) is well outside 1σ. The cross-discipline ablation deltas of +1.0 on aligned boundary and aligned failure subsets are categorical (every case in those subsets is blocked under the validator and none are blocked without it), giving a deterministic separation that does not require a confidence interval. <!-- TODO: if the reviewers want bootstrap CIs over the 50-query benchmark, see scripts/bootstrap_ci_rag.py -->
...
6. **Twelve omitted law-article rows.** Twelve of 3,102 law-article rows were excluded from the index because their statutory body text was empty in the source API response (§4.1). For most of these, the regulatory body is reachable through the parent Act on the National Law Information Center; back-filling them is an engineering follow-up rather than a research limitation. <!-- TODO: author to confirm whether to attempt PDF/HTML scrape of the 12 missing bodies for the camera-ready, or to leave the exclusion as documented -->
